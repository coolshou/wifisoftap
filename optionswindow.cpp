...
QDialog(parent), ui(new Ui::OptionsWindow) {
	...
	TestItem = false;
	while(1) { 
		temp_qstring = ui->driver->itemText(i);
		if(temp_qstring.length()>0) { 
			if(temp_qstring == GlobalSettings->value("AP/drive", "nl80211")) {
				ui->driver->setCurrentIndex(i); TestItem = true; 
			} 
		} else { 
			break;
		} i++; 
	...
	i=0;
	while(1) { 
		temp_qstring = ui->Channels->itemText(i);
		if(temp_qstring.length()>0) { 
			if(temp_qstring == GlobalSettings->value("AP/Channels", "7")) {  
	...
	if(temp_qstring.length()>0) { if(temp_qstring == GlobalSettings->value("AP/CountryCode", "US")) {  ui->CountryCode->setCurrentIndex(i); } } else { break; } i++; }

	/////////////////////////////////////////////
	// Mode
	/////////////////////////////////////////////
	i=0;while(1) { temp_qstring = ui->TypeAP->itemText(i);
	if(temp_qstring.length()>0) { if(temp_qstring == GlobalSettings->value("AP/TypeAP", "n")) {  ui->TypeAP->setCurrentIndex(i); } } else { break; } i++; }
	...
	// The third tab: dhcp
	/////////////////////////////////////////////
	ui->IP_CLIENT1->setText(GlobalSettings->value("DHCP/IP_CLIENT1", "192.168.110.2").toString());
	...
	TestItem = false;
	while(1) { 
		temp_qstring = ui->Internet_iface->itemText(i);
		if(temp_qstring.length()>0) { 
			if(temp_qstring == GlobalSettings->value("DHCP/Internet_iface", "eth0")) {  
				ui->Internet_iface->setCurrentIndex(i); TestItem = true; 
			} 
		} else { 
			break; 
		} 
...
// Save the settings from the second tab
void OptionsWindow::on_APSave_clicked(bool nomesseage = true)
{
	...
	if(ui->Password->text().length() > 7 && ui->Password->text().length() < 17) {
		...
		GlobalSettings->setValue("AP/Password", "My_WiFi_AP"); 
	}
	ui->Password->setText(GlobalSettings->value("AP/Password", "My_WiFi_AP").toString());
...
// Save the settings from the third tab
